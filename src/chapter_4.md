...
```dart
// ❌ Não faça - Provider com lógica de UI
class BadTaskProvider extends ChangeNotifier {
  void showSuccessDialog(BuildContext context) {
    showDialog(/* ... */);
  }
}

// ✅ Faça - Provider apenas com lógica de negócio
class GoodTaskProvider extends ChangeNotifier {
...
```dart
// ✅ Use Selector para propriedades específicas
Selector<UserModel, String>(
  selector: (context, user) => user.name,
  builder: (context, name, child) => Text(name),
)

// ✅ Use Consumer apenas onde necessário
Column(
  children: [
    Text('Título fixo'), // Não precisa de Consumer
...
// ✅ Use child parameter para widgets que não mudam
Consumer<CounterModel>(
  builder: (context, counter, child) {
    return Column(
      children: [
        Text('Contador: ${counter.value}'),
        child!, // Widget que não muda
      ],
    );
  },
  child: ExpensiveWidget(), // Criado apenas uma vez
)
```
...
class ApiProvider extends ChangeNotifier {
  ...
  Future<void> fetchItems() async {
    ...
    try {
      ...
      _setError('Erro do servidor: ${e.message}');
    } catch (e) {
...
void main() {
  group('TaskModel Tests', () {
    ...
    test('should add task correctly', () {
      ...
      // Assert
      expect(taskModel.tasks.length, 1);
      expect(taskModel.tasks.first.title, taskTitle);
      expect(taskModel.tasks.first.isCompleted, false);
    });

    test('should toggle task completion', () {
      // Arrange
      taskModel.addTask('Test Task');
      final taskId = taskModel.tasks.first.id;

      // Act
      taskModel.toggleTask(taskId);

      // Assert
      expect(taskModel.tasks.first.isCompleted, true);
    });
  });
}

// Widget Test com Provider
testWidgets('should display task count', (WidgetTester tester) async {
  ...
  await tester.pumpWidget(
    ChangeNotifierProvider<TaskModel>.value(
      value: taskModel,
      child: MaterialApp(
        home: Consumer<TaskModel>(
          builder: (context, model, child) {
            return Text('Tasks: ${model.totalCount}');
          },
        ),
      ),
    ),
